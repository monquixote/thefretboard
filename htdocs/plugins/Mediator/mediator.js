...
(function ($, undefined) {
	...
	function ReplaceGoogleMap (url, elem, $elem) {
		// get url variables //
		var splitUrl = url.split('?');
		// stuff to try and center the map //
		var firstCut = splitUrl[1].split("ssl="); // the deepest //
		var secondCut = firstCut[1].split("&");
		var centeringStuff = "&amp;ll=" + secondCut[0];
		var iframeSrc = splitUrl[0] + "?f=q&amp;source=s_q&amp;geocode=&amp;" + splitUrl[1].replace(/&/g, '&amp;') + "&amp;ie=UTF8&amp;hq=&amp;aq=" + centeringStuff + "&amp;iwloc=near&amp;output=embed";
		var newel = $('<iframe height="350" frameborder="0" scrolling="no" marginheight="0" marginwidth="0">');
